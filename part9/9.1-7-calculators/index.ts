...
app.post("/exercises", (req, res) => {
  ...
  if (!isNumberArray(exercises)) {
    res
      .status(400)
      .json({ error: "Malformatted parameters - daily_exercises should be array of numbers" });
    return;
  }
  if (exercises.length === 0) {
    res
      .status(400)
      .json({ error: "Invalid parameter daily_exercises - must include at least one day" });
    return;
  }
